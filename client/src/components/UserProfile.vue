...
<div>
        ...
        <v-layout>
          ...
          <v-tooltip bottom>
            <template v-slot:activator="{ on }">
              <v-icon :color="`${isOnline ? 'green' : 'grey'} lighten-2`" class="profile-status_icon mx-3 hidden-xs-only" medium v-on="on">mdi-circle</v-icon>
            </template>
            <span>{{ lastSeen }}</span>
          </v-tooltip>
          <v-chip disabled outlined small color="grey lighten-1" class="mt-3 ml-2 hidden-xs-only">{{ distance }}</v-chip>
          <v-btn icon text large color="primary" :disabled="userCantLike" @click="match"  class="hidden-xs-only">
          ...
          <v-speed-dial v-model="fab" direction="bottom" transition="slide-y-reverse-transition" class="speed_list">
            <template v-slot:activator>
              <v-btn icon text small large v-model="fab" color="primary">
  ...
  <v-dialog v-model="reportDialog" max-width="600px">
    <v-card class="pa-2">
      ...
      <v-card-actions>
        ...
        <v-btn color="primary" text @click="reportUser">Report</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
  <v-dialog v-model="blockDialog" max-width="600px">
    <v-card class="pa-2">
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="primary" text @click="blockDialog = false">Close</v-btn>
        <v-btn color="primary" text @click="block">Block</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
...
export default {
  ...
  computed: {
    ...
    liked: {
      get () {
        for (let match of this.following) if (match.id === this.user.id) return true
        return false
      },
      set () {
    ...
    likedBy () {
      for (let match of this.followers) if (match.id === this.user.id) return true
      return false
    },
    profileImage () {
  ...
  methods: {
    ...
    async match () {
      const url = `${process.env.URL}/api/matching/match`
      const data = {
    ...
    async block () {
      const url = `${process.env.URL}/api/users/block`
    ...
    async reportUser () {
      const url = `${process.env.URL}/api/users/report`
      let data = { id: this.$route.params.id }
      const headers = { 'x-auth-token': this.loggedIn.token }

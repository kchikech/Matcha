...
export default {
  ...
  data () {
    this.links = [
      { icon: 'mdi-account', text: 'Info', route: 'profile' },
      { icon: 'mdi-camera', text: 'Gallery', route: 'photo' }
    ]
    if (this.settings) {
      this.links[0].text = 'Profile'
      this.links.push({ icon: 'mdi-history', text: 'History', route: 'history' })
      this.links.push({ icon: 'mdi-account-cog', text: 'Parameters', route: 'setting' })
    }
    return {
      activeTab: null
    }

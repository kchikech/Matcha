    ...
    <v-flex :xl2="search" :xl4="!search" :md3="search" :md8="!search" sm10>
      <v-container px-md-5>
        <v-layout column>
          ...
          <v-text-field class="loaction_input mb-4" color="primary" outlined solo text append-icon="place" v-model="location"></v-text-field>
...
export default {
  ...
  async created () {
    ...
    const res = await this.$http.get(url, {filter: false}, { headers })
    if (!res.body.msg) {

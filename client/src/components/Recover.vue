...
<v-container class="recover mt-5">
  <v-form v-model="valid" class="my-4" v-if="notSubmited && !loading">
    <v-layout wrap justify-center>
      <v-flex xs12>
        <h1 class="page-header pass_reset_title display-3 mb-5 font-weight-light grey--text">Reset password</h1>
      </v-flex>
      <v-flex md8 sm10 xs12><v-text-field color="primary" v-model="password" validate-on-blur :rules="passRules" :counter="12"  label="New password" required autocomplete="off" :append-icon="showPass ? 'mdi-eye' : 'mdi-eye-off'" :type="showPass ? 'text' : 'password'" @click:append="showPass = !showPass"></v-text-field></v-flex>
      <v-flex md8 sm10 xs12><v-text-field color="primary" class="my-3" validate-on-blur :rules="confPassRules" v-model="passwordConfirm" :counter="12" label="Confirm new password" required autocomplete="off" :append-icon="showConfPass ? 'mdi-eye' : 'mdi-eye-off'" :type="showConfPass ? 'text' : 'password'" @click:append="showConfPass = !showConfPass" :error-messages="passwordMatch()"></v-text-field>
        </v-flex>
  ...
  <v-btn v-if="!notSubmited && !loading" large block rounded outlined router to="/" color="primary" class="mt-5 py-3"><span>Go back</span></v-btn>
  <loader v-if="loading"/>

...
<v-container class="mt-4">
    <div class="login mt-5">
        ...
        <v-form v-model="valid" class="my-4">
            ...
            <v-text-field @keyup.13="log" color="primary" class="my-5" v-model="password" validate-on-blur :rules="passRules" label="Password" required autocomplete="off" :append-icon="showPass ? 'mdi-eye' : 'mdi-eye-off'" :type="showPass ? 'text' : 'password'" @click:append="showPass = !showPass"></v-text-field>
            <v-btn block large depressed color="primary" dark @click.prevent="log" class="my-5">Login</v-btn>
            <v-layout row wrap justify-end>
                <v-btn text color="primary" dark to="/forgot">Forgot password</v-btn>
                <v-btn text color="primary" dark to="/register">Don't have an account? Sign up</v-btn>
            </v-layout>
        </v-form>
    </div>
...
export default {
  ...
  methods: {
    ...
    async log () {
      try {
        const url = `${process.env.URL}/api/auth/login`
        const auth = {
